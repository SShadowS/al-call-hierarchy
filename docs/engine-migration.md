...
| **Product wire-in — transaction-integrity GATE (approach B: Rust-native)** | The Rust engine's CLI `analyze` produces the gate's SARIF + PR-summary + exit-code + baseline natively (NOT a shell-out) — port the gate PATH into the engine, verified by TS-CLI-vs-Rust-CLI gate-output byte-parity | **SHIPPED — the GATE PATH is byte-identical to the al-sem TS CLI.** New `alsem` clap CLI (`src/bin/alsem.rs`) + `src/engine/gate/` (presets, projection→FindingSummary, filters [min-severity/scope/limit], format_sarif [SARIF 2.1.0], format_pr_summary [app-attribution], exit_code, preflight, inline_suppression [`al-sem-ignore` pragma], baseline [load/apply/save, fingerprint-keyed], model_instance_id [content-derived]). 3 stages (S1 SARIF, S2 PR-summary+exit-code+preflight, S3 suppression+baseline), each TS-CLI-oracle goldens + an in-process differential + a three-reviewer pass. Transaction-integrity preset = d8/d34/d35/d46/d47/d48/d49. The `--update-baseline` round-trip byte-matches (proves the edit-stable fingerprint). NEXT: broaden the CLI (terminal/html/json formats; fingerprint/digest/prove/diff/events/policy/explain/cache subcommands) toward full "Rust IS al-sem"; the Go-LSP wire-in (needs `al-language-server-go` checked out — DEFERRED); the al-perf re-point; the FINAL flip (merge `engine`→main, rename to al-sem, archive TS — ASK-first). Tracked: opt-in-union `--detector` (production-CLI fidelity; gate preset path byte-equivalent). |
