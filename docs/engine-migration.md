...
| **R4 (= R4-0…G + R4-F + d50 + cross-app)** | Full **L5 detector parity** — all 41 al-sem detectors reproduce al-sem's `Finding[]` at byte-parity | **COMPLETE — the Rust engine is ANALYSIS-COMPLETE: L0–L5 at byte-parity with al-sem + L4 incremental (Salsa).** All 41 detectors byte-match the full `ws-d*` corpus, `KNOWN_DIVERGENCES` empty, full `cargo test` green. The migration's analysis surface is fully ported. NEXT (post-analysis): the PRODUCT WIRE-IN — the transaction-integrity gate (D8/D34/D35/D46/D47/D48/D49, D47 the core; SARIF + PR-summary + exit-code/baseline contract; the Go LSP `publishDiagnostics`) drops the Rust engine in replacing TS; then the al-perf re-point; then the FINAL flip (merge `engine`→main, rename to al-sem, archive the TS repo — ASK first, irreversible) |
