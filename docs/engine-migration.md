...
| **R4-0…R4-G** | **L5 detector substrate + waves A–G** — the `DetectorContext` / path-walker / capability-query / transaction-spans / event-flow substrate + the `Finding`/`fingerprint`/`runDetectors` sort-dedup, then 34 detectors across waves A–G (intraprocedural, model-metadata, event/call-graph, capability-query, record-flow/parameterRoles, path-walker commit-reachability). Captured = the projected stable `Finding[]` from `runDetectors` (post-`analyzeWorkspace`, `deterministic:true`) per `ws-d*` fixture | **SHIPPED** — 34 detectors byte-parity over the per-detector `ws-d*` corpus (positive + negative), `KNOWN_DIVERGENCES` empty; two-stage opus review per wave; the stable-id `Finding` projection + `fingerprintOf`-over-internal-rootCauseKey + the `(detector, primaryLocationKey, rootCauseKey)` sort reproduced |
| **R4-F** | **The ordering-facts subsystem** — `engine/ordering-facts.ts` over the `snapshot/` + `digest/` + `ordering-engine` subsystem (the single largest R4 port; ~5k reachable LOC because `order:false` makes the P2 fact-graph dead). 6 stages: S0 root-classifications, S1 return-summaries, S2 snapshot (`composeSnapshot` consumed-core), S3 digest-effects + `occurrenceId`, S4 ordering-engine (HB/dominance + the 5 hazard labels → `ScopedGuarantee[]`), S5 facade + `gradeGuarantee` + d47/d49/d51. Each stage a per-stage `aldump` differential checkpoint | **SHIPPED** — d47 (the transaction-integrity gate's core), d49, d51 byte-match all 21 `ws-txn-d47/d49/d51` Finding goldens + every intermediate-stage golden (rootclass / return-summary / snapshot / digest-effects-with-occurrenceId / scoped-guarantees / ordering-facts), `KNOWN_DIVERGENCES` empty. External adversarial spec review (gpt-5.5 + claude-opus-4.6 + gemini-3.1-pro) folded as Revision 2; three-reviewer pass per stage; native-oracle hardening for thin-corpus branches. Load-bearing: the `occurrenceId = sha256(canonicalKey)[..16]` scheme, the witness-BFS first-shortest-path determinism, the M8 `localeCompare`-≠-ordinal finding (a faithful ICU/Bun collation for the ordering-facts key sort), `conditionality` provably unconsumed by the 5 labels. rootClassifications (S0) + `inherent_commit_behavior` (S1) are shared substrate that unblocked d50 |
| **R4 (d50 + cross-app d13/d16/d17 + exit gate)** | The final detectors: **d50** (checked-run-implicit-commit, over the S0/S1 substrate); **d13/d16/d17** (cross-app — a cross-app L5 entry: `DetectorContext` from `build_r4_cross_app_base` + `dep_routine_ids` role-scope filter + committed dep `.app` fixtures); the **R4 exit gate** | **SHIPPED — R4 COMPLETE.** **All 41 detectors byte-parity** over the full `ws-d*` corpus (positive + negative) + per-detector oracles, `KNOWN_DIVERGENCES` empty. d50 reused the R4-F S0/S1 substrate. The cross-app entry threads `dep_routine_ids` (role-scope filter drops dep-anchored findings). The d16 finding fingerprint required an al-sem **fix-then-freeze** (cache-INDEPENDENT fingerprint: stabilize dep routine ids in the rootCauseKey before hashing — only d16's golden moved, no cache bump) so the engine reproduces it cache-independently. Folded corpus-invisible faithfulness deltas (d17 implicit MS platform-tier deps, edgesByFrom-insertion-order sampling, cmpVersion parseInt semantics). |
| **R4 (= R4-0…G + R4-F + d50 + cross-app)** | Full **L5 detector parity** — all 41 al-sem detectors reproduce al-sem's `Finding[]` at byte-parity | **COMPLETE — the Rust engine is ANALYSIS-COMPLETE: L0–L5 at byte-parity with al-sem + L4 incremental (Salsa).** All 41 detectors byte-match the full `ws-d*` corpus, `KNOWN_DIVERGENCES` empty, full `cargo test` green. The migration's analysis surface is fully ported. |
| **Fingerprint edit-stability (fix-then-freeze)** | The finding fingerprint must be `modelInstanceId`-INDEPENDENT (edit-survivable) so the gate baseline survives workspace file add/remove | **SHIPPED** — `fingerprintOf`/`fingerprint_of` stabilize ALL routine ids (source + dep) in the rootCauseKey → StableRoutineId before hashing (extends the d16 dep-only fix). Only fingerprint fields moved (62 r4+gate goldens, both repos); no cache bump; MII-independence proven. The r4-finding fingerprint now equals the gate fingerprint for a shared finding. |
| **Product wire-in — transaction-integrity GATE (approach B: Rust-native)** | The Rust engine's CLI `analyze` produces the gate's SARIF + PR-summary + exit-code + baseline natively (NOT a shell-out) — port the gate PATH into the engine, verified by TS-CLI-vs-Rust-CLI gate-output byte-parity | **SHIPPED — the GATE PATH is byte-identical to the al-sem TS CLI.** New `alsem` clap CLI (`src/bin/alsem.rs`) + `src/engine/gate/` (presets, projection→FindingSummary, filters [min-severity/scope/limit], format_sarif [SARIF 2.1.0], format_pr_summary [app-attribution], exit_code, preflight, inline_suppression [`al-sem-ignore` pragma], baseline [load/apply/save, fingerprint-keyed], model_instance_id [content-derived]). 3 stages (S1 SARIF, S2 PR-summary+exit-code+preflight, S3 suppression+baseline), each TS-CLI-oracle goldens + an in-process differential + a three-reviewer pass. Transaction-integrity preset = d8/d34/d35/d46/d47/d48/d49. The `--update-baseline` round-trip byte-matches (proves the edit-stable fingerprint). Tracked: opt-in-union `--detector` (production-CLI fidelity; gate preset path byte-equivalent). |
| **CLI broadening A — `analyze` output formats (Rust-native)** | The remaining `analyze --format` outputs beyond the gate's sarif/pr-summary: **terminal**, **json** (the `DocumentEnvelope`), **html** — plus `--group-by` — byte-identical to the al-sem TS CLI. (`--dump-model` deliberately excluded — deprecated >500 MB debug path; rejected with CONFIG_ERROR.) | **SHIPPED — the `analyze` command is format-complete vs the TS CLI (sans the debug `--dump-model`).** 4 staged ports, each TS-CLI-oracle goldens (`scripts/cli-a-goldens/{stats,json,terminal,html}/`) + in-process committed-golden differential + a three-reviewer pass (2 opus + gemini-3.1-pro), `KNOWN_DIVERGENCES` empty. **A0** scaffold (`OutputFormat`+`Terminal/Json/Html`, `run_analyze` retains `L3Resolved`, `--group-by`/`--dump-model`, `alsem_version()`+`AL_SEM_VERSION_OVERRIDE`, `primary_app` thread). **A1** `detectorStats` byte-parity — added the `skipped` map (present-iff-nonzero) + per-detector candidate/skip emission across 41 detectors (the R4 surface never verified stats); fixed a corpus-invisible d1 `downgradedToInfo` post-merge-vs-per-direct-op bug found by review + native oracle; al-sem fix-then-freeze normalized d43's lone ungated `{other:0}`. **A2** the json `DocumentEnvelope` (unified canonical `serialize_document`: recursively-sorted keys, undefined-drop; `wrapAnalyzeReport`; `projectDiagnostics` `DIAG-<stage>`) — review found + fixed an incomplete 6-source diagnostics chain (provider→`discover`, the fail-closed `empty_output_result` dropped provider diagnostics) + non-deterministic `generatedAt` ms format. **A3** terminal + `rollupFindings` (IndexMap, non-total stable sort) + `--group-by`; 6 corpus-invisible render sub-branches locked by oracles. **A4** html (full-model masthead + inline SVG event-graph + per-finding evidence flow) — bezier int-division proven safe (provably-even coords); review found + fixed a `loc_key` raw-vs-projected co-location bug + oracle. Load-bearing: the `serialize_document` null-drop is shared with A1's stats serializer; the `str::cmp`/`trunc` UTF-16 gaps (non-BMP) are the known tracked engine-wide comparator follow-up, commented precisely at each site. NEXT: CLI broadening B (capability-query subcommands fingerprint/digest/prove/diff) then C (events/policy/cache); the Go-LSP wire-in (needs `al-language-server-go` checked out — DEFERRED); the al-perf re-point; the FINAL flip (merge `engine`→main, rename to al-sem, archive TS — ASK-first). |
| **CLI broadening B — capability-query subcommands (Rust-native, incl. binary CBOR/cbor.gz)** | `fingerprint` / `digest` / `prove` / `diff` byte-identical to the al-sem TS CLI across all formats — human/json/sarif AND the **binary `cbor` + `cbor.gz`** snapshot formats. (`digest --order` [the P2 inter-routine fact-graph] and `.app`-as-primary input deliberately EXCLUDED — rejected with CONFIG_ERROR, like `--dump-model`; tracked follow-ups.) | **SHIPPED — all four subcommands byte-parity, `KNOWN_DIVERGENCES` empty.** Spec Revision 2 included an empirical **feasibility proof** (Bun gzip = zlib-ng 2.3.3 @ level 6 → `flate2`+`zlib-ng`+OS-byte patch; cbor-x → a hand-rolled encoder with the `0xb9`-always-map-16 / insertion-order-keys / always-f64 / keep-undefined rules). 5 staged ports (`scripts/cli-b-goldens/`), each TS-CLI-oracle goldens + committed-golden differential + a three-reviewer pass (2 opus + gemini), corpus-invisible bugs locked by native oracles throughout. **B0** snapshot completion + the 4 canonical serializers (raw-JSON sorted, envelope-JSON, **hand-rolled CBOR**, **gzip via zlib-ng** with OS-byte normalization, sharding) + workspaceFingerprint + the `locale_compare` ICU comparator at 7 deriver sorts; the CBOR int>2³²→f64 + map-count guard. **B1** digest (unresolved-cone + conditionality, dead under R4-F `order:false`) — review fixed a `--diff` parser deleted/rename bug + exit-code (1-not-2/3) + `rootsRequested` undercount + a nondeterministic selector. **B2** prove (tristate absence-safety) — review fixed a **false-absence-proof soundness bug** (gaps-in-cone missing the app-boundary prefix) + the symbol-anchor gate. **B3** fingerprint (query + witness modes + human compact/full + the cbor/shard dispatch) — the largest review pass: fixed the `--witness 3`/`--include-inherited` flag semantics, `--roots` validation, 9 witness-diagnostic kinds, raw-hop human rendering, per-mode stderr, + a ~120-line BFS de-dup; 39 native oracles. **B4** diff (snapshot deserialize [CBOR decoder + gunzip] + the 5 passes ABI/schema/events/capabilities/permissions + human/json-envelope/sarif + renames/coverage-policy) — review fixed a CBOR-decoder unbounded-recursion abort (no-panic breach) + rename/preflight/severity oracles. Load-bearing: the binary CBOR/gzip byte-parity held green from B0's encoder through B4's decoder (round-trip oracle); the snapshot top-level key order = `compose.ts` literal (CBOR is insertion-order-sensitive). NEXT: CLI broadening C (events/policy/cache); the Go-LSP wire-in (needs `al-language-server-go` checked out — DEFERRED); the al-perf re-point; the FINAL flip (ASK-first). Tracked: `digest --order` P2 fact-graph + `.app`-as-primary input (both excluded, reject-cleanly); the engine-wide UTF-16 comparator swap (`str::cmp`/`trunc`/sort sites). |
| **CLI broadening C — events / policy / cache (Rust-native)** | `events {fanout,chains}`, `policy {check,explain}` (a full predicate engine + YAML policy loader), `cache prune` — byte-identical to the al-sem TS CLI over the byte-testable surface. | **SHIPPED — `KNOWN_DIVERGENCES` empty.** 3 staged ports (`scripts/cli-c-goldens/`), each TS-CLI-oracle goldens + committed-golden differential + a three-reviewer pass (2 opus + gemini), corpus-invisible bugs locked by native oracles. **C1 events** — a custom INSERTION-ORDER JSON serializer (NOT the DocumentEnvelope) + unicode-glyph human render over the R4-ported `event_flow.rs`; review caught a dead `--coverage-policy strict/ignore` (validated-but-inert) → ported the partial-entry filter + `coverage-incomplete:` stderr + exit-1 + the summary-passthrough quirk (+ a partial-coverage fixture). **C2 policy** — the largest new piece: a `serde_yaml` loader (string-only values, version==1, dup-key/alias reject, vendored byte-identical `policy-default.yaml`) + the predicate compiler/evaluator (4 node kinds, 4 operators, **16 fields**, the 3 Kleene truth tables, `onUnknown`/coverage-gate/applicability-two-mode) + the policy engine reusing `fingerprint_of`+stableRootCauseKey; a STEP-0 model-input parity check (rootClassifications + capabilityFacts + coverage) preceded the port. Review caught a **glob ReDoS + stack-overflow** (hard-rule breach) → rewrote the matcher onto the non-backtracking `regex` crate with the JS-faithful translation (`*`→`[^\n\r  ]*`, ASCII-case-insensitive), preserving the glob-vs-glob-in array asymmetry; + version coercion, a vendored-YAML parity guard, and a shared `ordered_json` serializer (de-duped across events/policy/B-stages). **C3 cache prune** — literal-string-replacement content hash + the 6-step classification (CACHE_VERSIONS tuple matching al-sem, override-aware analyzer stamp) + sorted dry-run accumulation; scoped honestly (classification native oracles + a `--dry-run` byte-differential + an integration delete-test — the mutation is not byte-differentialed); review fixed the unlink-failure accounting + oracle'd the classification edges (junk→bad-name, extra-version-key→kept, `.tmp.`-substring-skip, summaryMode/precedence). **★ The Rust `alsem` CLI is now the FULL al-sem command set at byte-parity** (analyze all formats + fingerprint/digest/prove/diff incl. binary CBOR/cbor.gz + events/policy/cache), sans the deliberately-excluded `--dump-model`/`--order`/`.app`. NEXT: the Go-LSP wire-in (needs `al-language-server-go` checked out — DEFERRED); the al-perf re-point; the FINAL flip (merge `engine`→main, rename to al-sem, archive TS — ASK-first). |
| **al-perf fusion support — `fingerprint --inventory-only` (engine-side)** | A NEW al-perf-facing output (al-sem TS has no equivalent): a lean projection of the capability-snapshot — `apps` + the per-routine inventory (`objectType`/`objectNumber`/`routineName` + StableRoutineId) + `identities` + `coverage` + `rootClassifications`, omitting the heavy consumed-core — so al-perf can correlate runtime hotspots with al-sem's routine universe without parsing a 100MB snapshot. | **SHIPPED (engine side of al-perf P1 fusion).** `fingerprint --inventory-only --format json` → a `routine-inventory` envelope (schemaVersion 1.0.0). NOT a byte-parity-with-TS port (greenfield) — tested as a projection-SUBSET self-consistency (its `apps`/`identities`/`coverage`/`rootClassifications` are byte-identical to the full snapshot's; the heavy keys absent) + a combo-rejection (`--inventory-only` + query-selectors/cbor). The al-perf consumer (branch `feat/alsem-fusion` in the al-perf repo) shells out to this + `analyze --format json` and correlates by `(objectType, objectNumber, routineName)` to fuse runtime hotspots with static findings — P1 (substrate) complete; P2 (the hotspot↔finding UX + runtime-prioritized findings) next. |
