...
- **d22 FN [MED]**: implicit-trigger `Rec` FlowField reads invisible — `field_accesses` only recorded for declared `record_var_names`, not the implicit-Rec base frame (body_walk.rs:894-899) → `Rec."Balance (LCY)"` in OnAfterGetRecord generates no PFieldAccess → d22 silent. **DEFERRED (own session)**: the fix recording implicit-base-receiver field accesses lives in the SHARED L2 capture (body_walk implicit_receiver_stack) and feeds d3 + d22 + the L4 field-access summaries — a wide golden rebaseline + FP re-verification across d3/summaries. Real + valuable FN, but too invasive to land safely at the tail of this batch; warrants a dedicated TDD plan (capture implicit-Rec/bare-field reads → verify d3 unaffected → rebaseline).
...
7. **FN**: ~~d20 break~~ (DONE), d22 implicit-Rec FlowField (DEFERRED — wide L2-capture rebaseline), ~~d37 ModifyAll-not-persist~~ (DONE) — premise-narrowing fixes.
