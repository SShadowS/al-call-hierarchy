...
//! matching, scoped to `test == R2_5B_RT_TEST_NAME`. Target: empty.
...
const R2_5B_RT_TEST_NAME: &str = "differential_r2_5b_record_types_match_goldens";
...
/// R2.5b-rt-scoped entries apply here.
...
fn differential_r2_5b_record_types_match_goldens() {
    ...
    let allowlist: Vec<AllowEntry> = load_allowlist()
        .into_iter()
        .filter(|e| e.test == R2_5B_RT_TEST_NAME)
        .collect();
    ...
    // --- Allowlist gating (same semantics as R0/R2a) ------------------------
    ...
    if !undocumented.is_empty() {
        failure.push_str(&format!(
            "\n{} UNDOCUMENTED R2.5b-rt divergence(s) (not in KNOWN_DIVERGENCES.json, \
             test={R2_5B_RT_TEST_NAME}):\n",
            undocumented.len()
        ));
    ...
    if !unused.is_empty() {
        failure.push_str(&format!(
            "\n{} UNUSED R2.5b-rt allowlist entr(y/ies) (no matching divergence this run):\n",
            unused.len()
        ));
        for e in &unused {
            failure.push_str(&format!(
