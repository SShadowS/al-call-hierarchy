...
//!   - the opaque-vs-external-target split (R3a-0, real ledger threaded DURING resolve —
//!     matches fixed al-sem): on the ALL-FETCHED corpus a member call to an ABSENT object
//!     is `external-target` GENUINELY (all declared deps fetched →
//!     `has_unfetched_declared_dependency` false), and ONLY the object-run form into an
//!     absent declared dep is `opaque` (ledger-independent). The unfetched-declared-dep
//!     member-`opaque` branch (Fix 1) is proven out-of-corpus by
...
// ============================================================================
// 5. The opaque-vs-external-target split (R3a-0, real ledger threaded): on the
//    ALL-FETCHED corpus a MEMBER call to an absent object is `external-target`
//    GENUINELY (all declared deps fetched → has_unfetched_declared_dependency false);
//    ONLY the object-run form into an absent declared dep is `opaque`. The
//    unfetched-declared-dep member-`opaque` branch (Fix 1) is proven out-of-corpus by
//    tests/r3a0_unfetched_dep_opaque.rs.
// ============================================================================
...
fn opaque_is_object_run_only_and_member_miss_is_external_target() {
    ...
    // NO member-call edge is `opaque` on the all-fetched corpus (all declared deps
    // fetched → has_unfetched_declared_dependency false). The member-opaque branch fires
    // ONLY for an UNfetched declared dep, proven in tests/r3a0_unfetched_dep_opaque.rs.
    ...
    assert_eq!(
        member_opaque, 0,
        "NO member call is opaque on the all-fetched corpus (gone.M() is external-target; \
         the unfetched-dep member-opaque branch is proven in r3a0_unfetched_dep_opaque.rs)",
    );
