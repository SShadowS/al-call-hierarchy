//! D21 — read on uninitialised record. Port of al-sem
//! `src/detectors/d21-read-without-load.ts`.
//!
...
//! by-var parameter records (caller-loaded). G-10/G-16: a prior call that
//! loads the record also counts — the platform built-in `GetBySystemId` and
//! any resolved callee chain that loads the by-var argument (bounded multi-hop
...
pub fn detect_d21(resolved: &L3Resolved, ctx: &DetectorContext) -> DetectorOutput {
    ...
    for routine in &ws.routines {
        ...
        for op in &routine.record_operations {
            if !READING_OPS.contains(&op.op.as_str()) {
                continue;
            }
            ...
            let path = vec![EvidenceStep {
                ...
                note: format!(
                    "{} on {} with no prior Get/Find/Init in this routine",
                    op.op, op.record_variable_name
                ),
            }];

            let id = format!("d21/{}/{}", routine.id, op.id);
            ...
            let root_cause = format!(
                "{} calls {} on {} but never loaded it — the call operates on a default record \
                 and may silently succeed or always raise.",
